...
- **Attack Mode:** The monster uses its ATK. If it defeats a monster in attack mode, the difference reduces the opponent's LP.
- **Defense Mode:** The monster uses its DEF. If it is attacked and its DEF is higher than the enemy's ATK, the attacker takes damage equal to the difference. If it is lower, the monster is destroyed, but the owner loses no LP.
- **Total LP:** Each duelist starts with 600 Life Points.

### 4.2. Mana and Turn Management

- **Starting Hand:** 5 cards (maximum of 6 cards in hand).
- **Draw per Turn:** 1 card.
- **Mana:** Each player starts with 5 Mana Points on the first turn. Mana increases by 2 every turn (no maximum limit defined yet).

### 4.3. Win/Loss Conditions

A duel is resolved when:

- **Victory:** Opponent LP reaches 0.
- **Defeat:** Player LP reaches 0 or a **Deck Out** occurs (required draw with an empty deck).
...
## 6. Priority Artificial Intelligence (AI)
...
- **Bottom Left:** Displays LP (600) and the NPC/Player name.
...
| --------------- | -------------------------------------------- | --------------------------------------------------------------------------------------------------------------- |
| **1. Draw**     | None (System Action).                        | The system draws 1 card from the Deck to the Hand.                                                              |
...
## 10. Graveyard Mechanics

With a lean 20-card deck, resource management is a critical factor for victory.

- **Discard Flow:** Used Spells, activated Traps, and destroyed Monsters are moved to the **Graveyard**.
- **Strategic Interactivity**: The **Graveyard** is a public zone. Both the player and NPC can check on either Graveyard pile at any time to inspect discarded cards, allowing for tactical planning and tracking of used resources.
- **Finite Resource System**: Once a card is sent to the **Graveyard**, it remains there for the duration of duel unless retrieved by specific card effect (e.g., Revive).
...
- No damage is subtracted from either player's LP in this situation.
...
- **Visual Feedback:** Cards must have clear visual states for _Attack Mode (Vertical)_, _Defense Mode (Horizontal)_ and _Hidden Card (Face-down)_.
